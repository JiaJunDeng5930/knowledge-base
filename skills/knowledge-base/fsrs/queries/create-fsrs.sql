...
with input as (
    ...
    insert into public.fsrs_knowledge (fsrs_id, record_id)
    select distinct created.id, record_id.value::bigint
    from created
    cross join input
    cross join lateral jsonb_array_elements_text(input.data -> 'record_ids') as record_id(value)
    returning fsrs_id
)
select created.id
from created
