...
优先读取当前仓库检出。skill 单独安装时，通过用户已连接的 GitHub 取得上述文件，例如 [知识整理 skill](https://github.com/JiaJunDeng5930/knowledge-base/blob/main/skills/scenarios/integrate-explained-knowledge/SKILL.md)。在已有访问权限内自动读取所需资料，不要求用户手动调用这些模块。模块提供本任务需要的知识整理规则和数据接口；其中的建库、权限配置、删除和正式提交等操作只在用户另外授权相应任务时适用。知识整理规则与草稿保存接口不能相互替代。

先取得依赖再执行相应操作。临时网络故障可通过同一接口重试；权限或安全检查拒绝时停止相应操作，不切换身份、凭据或访问方式规避拒绝。某项依赖不可用时保留待处理结果，继续能完成的部分；知识整理 skill 尚未读到时不写入知识草稿。
